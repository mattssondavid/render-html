...
describe('render', (): void => {
    ...
    //         // prettier-ignore
    //         return html`${content}<p>${footer}</p>`;
    //     };
    //     render(template('1', '2'), container);
    //     assertEquals(container.innerHTML, `<h1>1</h1><p>content</p><p>2</p>`);
    //     // Render update
    //     render(template('1', '3'), container);
    //     assertEquals(container.innerHTML, `<h1>1</h1><p>content</p><p>3</p>`);
    // });

    // it('can handle rendering single nested template part where one has attribute', (): void => {
